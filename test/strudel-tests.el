...
(ert-deftest strudel-first-evaluation-starts-and-plays-only-latest ()
  (let ((strudel--project nil) (strudel--buffer nil) (strudel--widget nil)
        (strudel--state "off") (strudel--pending-eval nil)
        (strudel--sources (make-hash-table :test #'eql))
        (directory (file-name-as-directory (file-truename default-directory)))
        (starts 0) sent)
    (cl-letf (((symbol-function 'strudel-start)
               (lambda (dir)
                 (cl-incf starts)
                 (setq strudel--project dir strudel--buffer (current-buffer)
                       strudel--widget 'test strudel--state "loading")))
              ((symbol-function 'strudel--send)
               (lambda (command) (push command sent))))
      (strudel-eval-string "first" directory "song.org:1")
      (strudel-eval-string "latest" directory "song.org:5")
      (should (= starts 1))
      (should-not sent)
      (strudel--receive 'old "{\"state\":\"ready\",\"events\":[]}")
      (should-not sent)
      (dotimes (_ 2)
        (strudel--receive 'test "{\"state\":\"ready\",\"events\":[]}"))
      (should (= (length sent) 1))
      (should (equal (alist-get 'code (car sent)) "latest"))
      (should (equal (gethash (alist-get 'id (car sent)) strudel--sources) "song.org:5"))
      (should-not strudel--pending-eval)
      (strudel-eval-string "update" directory)
      (should (= starts 1))
      (should (equal (alist-get 'code (car sent)) "update")))))

(ert-deftest strudel-pending-evaluation-can-be-cancelled ()
  (dolist (action '(stop player-stop error cleanup))
    (let ((strudel--project default-directory) (strudel--buffer (current-buffer))
          (strudel--widget 'test) (strudel--state "starting-audio")
          (strudel--pending-eval (list "pending" default-directory "song.org"))
          (strudel--sources (make-hash-table :test #'eql))
          (strudel--poll-timer nil) sent)
      (cl-letf (((symbol-function 'strudel--send)
                 (lambda (command) (push command sent)))
                ((symbol-function 'strudel--server-stop) #'ignore))
        (pcase action
          ('stop (strudel-stop))
          ('player-stop
           (strudel--receive 'test "{\"state\":\"ready\",\"events\":[{\"type\":\"stopped\"}]}"))
          ('error (strudel--receive 'test "{\"state\":\"error\",\"events\":[]}"))
          ('cleanup (strudel--cleanup)))
        (should-not strudel--pending-eval)
        (strudel--receive 'test "{\"state\":\"ready\",\"events\":[]}")
        (should-not sent)))))

(ert-deftest strudel-evaluation-switches-project-and-propagates-startup-errors ()
  (let ((strudel--project "/old-project/") (strudel--buffer (current-buffer))
        (strudel--pending-eval '("old" "/old-project/" "old.org"))
        (strudel--state "loading")
        (directory (file-name-as-directory (file-truename default-directory)))
        started)
    (cl-letf (((symbol-function 'strudel-start)
               (lambda (dir) (setq started dir strudel--pending-eval nil))))
      (strudel-eval-string "new" directory "new.org")
      (should (equal started directory))
      (should (equal strudel--pending-eval (list "new" directory "new.org"))))
    (setq strudel--pending-eval nil)
    (cl-letf (((symbol-function 'strudel-start)
               (lambda (_) (user-error "Run strudel-setup"))))
      (should-error (strudel-eval-string "new" directory) :type 'user-error)
      (should-not strudel--pending-eval))))
